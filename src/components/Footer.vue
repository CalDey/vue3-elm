<template>
    <div>
        <section id="footer" class="fixed left-0 right-0 bottom-0 flex w-full h-10 z-50 bg-white text-gray-400">
            <!-- 引入所需SVG -->
            ...
            <!-- 底部Tabbar -->
            <!-- 外卖 -->
            <section @click="gotoAddress({path: '/msite', query: {geohash}})" class="flex flex-col flex-1 justify-center items-center">
                <svg class="w-4 h-4 fill-current">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('msite') !== -1 ? '#msiteActive' : '#msite'"></use>
                </svg>
                <span class="text-xxs">外卖</span>
            </section>
            <!-- 搜索 -->
            <section @click="gotoAddress({path: `/search/${geohash}`})" class="flex flex-col flex-1 justify-center items-center">
                <svg class="w-4 h-4 fill-current">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('search') !== -1? '#findActive' : '#find'"></use>
                </svg>
                <span class="text-xxs">搜索</span>
            </section>
            <!-- 订单 -->
            <section @click="gotoAddress({path: '/order'})" class="flex flex-col flex-1 justify-center items-center">
                <svg class="w-4 h-4 fill-current">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('order') !== -1? '#orderActive' : '#order'"></use>
                </svg>
                <span class="text-xxs">订单</span>
            </section>
            <!-- 我的 -->
            <section @click="gotoAddress({path: '/profile'})" class="flex flex-col flex-1 justify-center items-center">
                <svg class="w-4 h-4 fill-current">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('profile') !== -1? '#profileActive' : '#profile'"></use>
                </svg>
                <span class="text-xxs">我的</span>
            </section>
        </section>
    </div>
</template>
...
export default {
    ...
    setup() {
        ...
        // console.log(store.state.geohash)
